...
def mosaic(layer_name1, layer_name2, outpath_dataname):
    ...
    data_ok = driver.Create(outpath_dataname, 9600, 4800, 1, gdal.GDT_Float64, [
                            "COMPRESS=DEFLATE"])  # 创建数据集
    #print (data)
...
def choose_data(filepath, out_path):
    ...
    for i in filepath:  # 遍历数据列表、
        ...
        if str_i.endswith("hdf"):
            str1_name = []
            # 数据名：MOD13Q1.A2016001.h26v05.006.2016029065706
            str1_name = str_i.split('.')
            for n in filepath:
                str_n = str(n)
                # 排除两个数据相同的情况
                if str_n == str_i and os.path.exists("E:\Python\data\mosaic_data\\"+str1_name[1]+".tif"):
                    print("数据相同"+str_i)
                    continue
                else:
                    if (str_n.endswith("hdf")):
                        ...
                        if str1_name[0] == str2_name[0] and str1_name[1] == str2_name[1] and str1_name[3] == str2_name[3]:  # 判断名称
                            ...
                            if v1 == v2 and (int(h1) == (int(h2)+1)):  # 判断行列号是否相近
                                ...
                                outpath_dataname = os.path.join(
                                    out_path, str1_name[1]+".tif")
                                print("左右拼")
                                mosaic(layer_name2, layer_name1,
                                       outpath_dataname)
                            if v1 == v2 and (int(h1) == (int(h2)-1)):  # 判断行列号是否相近
                                # data_path1=os.path.join( path,m)
                                # data_path2=os.path.join( path,n)
                                ...
                                outpath_dataname = os.path.join(
                                    out_path, str1_name[1]+".tif")
                                print("右左拼")
                                mosaic(layer_name1, layer_name2,
                                       outpath_dataname)
