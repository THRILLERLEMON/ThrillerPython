...
# 参量_年份
parszhang = ["有效灌溉面积"]
targetPath = 'E:\\OFFICE\EXCEL 数据处理\\Data2.0'
path = 'E:\\OFFICE\\EXCEL 数据处理\\FillBlank'
onetoN_Code = pd.read_excel('E:\\OFFICE\\EXCEL 数据处理\\Add1516Data\\OnetoN_Code.xlsx', sheet_name="Code")

# 循环每一个参数
for i, par in enumerate(parszhang):
    allCountryData = pd.read_excel(targetPath + '\\各变量RES0810.xls', sheet_name=par)
    xlsList = FindXLSfile(par, path)
    # 循环每一个县
    for c, oneCountryCode in enumerate(allCountryData.loc[:, "County_code_N"]):
        print('Find' + str(oneCountryCode))
        if pd.isnull(oneCountryCode):
            break
        # 循环每一年
        for yearn in np.arange(2000, 2017, 1):
            filedname = par + "_" + str(yearn)
            # First,to identify the value is null
            if np.isnan(allCountryData.loc[c, filedname]):
                # 对城区的代码进行加和处理
    ...
    allCountryData.to_excel(targetPath + '//' + par + '.xls')
    print('完成 ' + par + '所有值的寻找')
